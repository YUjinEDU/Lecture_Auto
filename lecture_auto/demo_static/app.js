...
function renderHome() {
      ...
      <div class="lecture-card" data-status="${st}" data-job="${job.job_id}">
        <div class="card-status-row">
          ...
          <div class="card-actions">
            ${['queued','stopped','failed','error'].includes(st) ? `
            <button class="card-action-btn card-restart-btn" data-job="${job.job_id}" title="다시 시작" aria-label="다시 시작">
  ...
  // Restart buttons (queued/stopped/failed jobs)
  lectureGrid.querySelectorAll('.card-restart-btn').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      await api(`/demo/api/jobs/${btn.dataset.job}/rerun`, { method: 'POST' });
      navigateToDetail(btn.dataset.job);
    });
  });
}
