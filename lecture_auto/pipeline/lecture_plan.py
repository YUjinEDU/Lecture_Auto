...
def build_section_prompt(
    ...
    lines.append(
        "- 음성 합성(TTS) 안정성을 위한 문장 호흡 원칙: 각 문장은 너무 길거나 짧지 않게, "
        "1문장당 70~90자 내외(또는 40~50자 단문 2개 결합)로 작성한다. "
        "100자를 초과하는 긴 만연체 복문(음성 늘어짐/어눌함 유발)이나 30자 미만의 단편 문장(음성 잘림 유발)을 피하고 자연스럽고 또렷한 호흡을 유지한다."
    )
    if is_last_section:
        lines.append(
