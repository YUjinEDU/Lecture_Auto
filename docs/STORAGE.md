...
기존 결과물(2026-09-23 이전 완료): `01_…/01_….mp4`, `04_…/종합설계_02_고객문제이해및정의_인트로교체.mp4`(교수님 전달본, 인트로 수동 교체 — D-13) + `_ai_backup.mp4`(AI 원본).
04는 전달본 이름이 표준 이름이 아니라 `--status`에 "영상 없음"으로 나온다(표준 이름으로 조립하면 전달본을 덮지 않도록 일부러 둠). 01·04 음성은 S6 이전 제작이라 `qc없음`으로 표시된다.
`output/_archive/`: 테스트 영상(mini 3slides).
